...
| **P3 AI 루브릭 채점** ✅ | `/api/grade` + 가중합 | **구현 완료(2026-06-24)** — `grader`(추상화)+`geminiGrader`(generateObject·jsonSchema, zod 미도입)+`score`(정규화·가중합)+`requestValidation`, `/api/grade`가 `ChallengeGradingResult` 산출. 학생 플로우 연결: `ChallengeSolveView` 상단 '제출' → `useGradeChallenge`(testResult 없으면 `runTests()` 선행) → 변경 파일만 전송 → `ChallengeGradingResultPanel` 모달 표시. 라우트는 채점기 실패를 잡아 읽을 수 있는 502로 반환. 검증: typecheck·lint·단위테스트(34) 통과, 라이브 `/api/grade`가 검증(400)·400 경계·페이지 컴파일(200) 확인, 실 Gemini 호출까지 도달 확인. ⚠️ **성공 200 채점 결과는 Gemini 무료 티어 일일 쿼터 소진(limit 20)으로 미실측** — 코드 결함 아님, 쿼터 회복/유료 키로 재확인 필요. |
