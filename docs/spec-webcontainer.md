...
WebContainer를 도입해도 **백엔드 최소화 원칙은 유지된다.** 코드 실행은 전부 브라우저(WebContainer)에서
일어나고, 서버 라우트는 여전히 **AI 프록시 / 채점용 LLM 프록시 = 키 은닉 + 정책 주입** 용도뿐이다.
...
핵심: 코드 실행·테스트는 브라우저(WebContainer), 서버는 LLM 프록시 2개(agent·grade)뿐.
```
...
| **P1 워크스페이스** | Monaco ↔ FS 동기화 + AI 미러링 연결 | 학생/AI가 코드 수정 → 미리보기 HMR 갱신 |
...
| **채점 무결성** | 채점이 클라이언트에서 실행→점수 위조·테스트파일 열람 가능 | 제출 시 서버 샌드박스 재실행(하이브리드) 검토 — 아래 결정 6 |
...
### 추가 결정 필요 (P1 진입 전 확정)
5. **COEP 모드** — `credentialless`(편하지만 Safari 약함) vs `require-corp`+자체호스팅(견고). §2.2.
6. **채점 무결성 수준** — (a) 클라이언트 신뢰 / (b) 제출 시 서버 샌드박스(e2b 등) 재실행 하이브리드.
   진짜 시험 신뢰성이 필요하면 (b). §7.2.
...
| CLAUDE.md 원칙 | 부합 여부 |
|---|---|
| 백엔드 최소화 | ✅ 실행은 브라우저, 서버는 LLM 프록시 2개뿐 |
| MongoDB 지양 | ✅ 계속 미사용, 상태는 클라이언트 |
