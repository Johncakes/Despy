...
> 작성일: 2026-06-25 · 상태: **초안(Draft) — 검토/승인 대기**
> 목적: 현재 MVP(무서버 세션·localStorage·인증 없음)를 **실제 운영 가능한 서비스**로 전환하기 위한
> 요구사항을 정의한다. 핵심 축은 ① **인증(세종대 로그인 연동)**, ② **인가(교수 접근 관리)**,
> ③ **데이터 영속화(DB 도입)**, ④ **멀티유저/동시성·채점 무결성**이다.
>
> ⚠️ 이 문서는 **명세(무엇을·왜)** 이며, 구현(어떻게)은 §11 로드맵과 §12 미결정 사항을
> 확정한 뒤 시작한다. 아키텍처·DB·라이브러리·데이터 모델 변경은 CLAUDE.md 기준 **Blocking** —
> §12를 승인받기 전에는 코드 작업을 시작하지 않는다.
...
| 단계 | 내용 | 산출물 |
|---|---|---|
| **M0 결정** | §12 Blocking 항목 확정(DB·세종 통합 방식·교수 승격) | 승인된 결정 기록 |
| **M1 DB 기반** | DB 도입·스키마·클라이언트 싱글턴·마이그레이션 도구 | `users`/`problems` 등 테이블 |
| **M2 인증** | 세종 인증 연동(`/api/auth/*`) + 세션 + 자격증명 비저장 | 로그인/로그아웃/세션 |
| **M3 인가** | RBAC·라우트 가드·소유권 검사·교수 승격 | 역할별 접근 통제 |
| **M4 영속 이전** | 문제/과제/풀이/제출/채점 서버 CRUD + Query 전환 | 멀티유저 데이터 |
| **M5 무결성** | 민감정보 서버 이전·AI 한도 서버 강제·채점 서버 권위 강화 | 변조 불가 통제 |
| **M6 운영** | 감사로그·레이트리밋·PIPA 고지·관측성·세종 협의 | 운영 준비 완료 |

---

## 12. 미결정 사항 (Blocking — 결정 후 착수)

> CLAUDE.md상 아키텍처·DB·라이브러리·데이터 모델·권한 변경은 **결정 전 착수 금지**.

- **D-A. DB 선택**: Postgres(권장: 관계·트랜잭션·RBAC 적합) vs MongoDB(스타터 잔재 활용) vs Supabase/PlanetScale(관리형) vs SQLite. → ORM/마이그레이션 도구 동반 결정.
- **D-B. 세종 인증 통합 방식**: (A) TS 자체 포팅 vs (B) Python 사이드카(MIT) vs (C) Java 사이드카. (B/C는 새 서비스 → 운영 영향 큼)
- **D-C. 세션 메커니즘**: 서버 세션(쿠키+DB/Redis) vs Stateless JWT. (라이브러리 도입 여부 포함: 예) Auth.js/Lucia)
- **D-D. 교수 승격 방식**: 관리자 수동 / 사번·이메일 allowlist / 초대코드 / 교수 자체계정 대체경로 — 택1+조합.
- **D-E. 세종대 공식 협의 여부**: 비공식 모방 인증의 약관·법적 리스크 수용 범위. (운영 전 학교 문의 권고)
- **D-F. 호스팅/배포**: 세션·DB·(사이드카) 운영 환경(Vercel 단독으로 사이드카 운영 제약 등).
